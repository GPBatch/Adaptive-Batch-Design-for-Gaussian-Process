...
lik.fh.optimizef = @lik_probit_optimizef;
...
function [f, a] = lik_probit_optimizef(gp, y, K)
tol = 1e-8;
maxiter = 200;
n = size(y,1);
f = zeros(size(y,1),1);
...
dlp = gp.lik.fh.llg(gp.lik, y, f, 'latent', []);
lp_new = gp.lik.fh.ll(gp.lik, y, f, []);
lp_old = -Inf;
if issparse(K)
  speyen=speye(n);
end
