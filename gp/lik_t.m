...
% set(lik.fh.llg,'callback',{@lik_t_llg_callback,lik.fh})
...
%   ip.addParamValue('sigma2',0.1, @(x) isscalar(x) && x>0);
  ip.addParamValue('sigma2', 0.1, @(x) (isvector(x) && all(x>=0)) || (isscalar(x) && x >= 0));
...
%   ip.addParamValue('nu_prior',prior_fixed, @(x) isstruct(x) || isempty(x));
  ip.addParamValue('nu_prior',prior_t(), @(x) isstruct(x) || isempty(x));
  ip.parse(varargin{:});
  lik=ip.Results.lik;
...
%    LIK_T_LL, LIK_T_LLG2, LIK_T_LLG3, GPLA_E
  ...
  r = y-f;
  v = lik.nu;
  if (size(lik.sigma2, 1) == 1)
      sigma2 = lik.sigma2;
  else
      ind = 1:size(r, 1);
      sigma2 = lik.sigma2(ind);
  end
...
%       llg2 = llg2.*sigma2;
      ...
      if ~isempty(lik.p.sigma2)
        i1=i1+1;
        % Derivative with respect to sigma2
        llg2(:, i1) = -v.*(v+1).*r ./ (v.*sigma2 + r.^2).^2;
        % correction for the log transformation
        llg2(:, i1) = llg2(:, i1).*sigma2;
      end
      if ~isempty(lik.p.nu)
        % gradient d^2 / (dfdnu)
        i1 = i1 + 1;
        llg2(:,i1) = r./(v.*sigma2 + r.^2) - sigma2.*(v+1).*r./(v.*sigma2 + r.^2).^2;

        % Correction for the log transformation
        llg2(:, i1) = llg2(:,i1).*v.*log(v);
      end
  end
end    
...
%       llg3 = llg3.*sigma2;
      ...
      if ~isempty(lik.p.sigma2)
        i1=i1+1;
        % Derivative with respect to sigma2
        llg3(:, i1) = (v+1).*v.*( v.*sigma2 - 3.*r.^2) ./ (v.*sigma2 + r.^2).^3;
        % correction for the log transformation
        llg3(:, i1) = llg3(:, i1).*sigma2;
      end
      if ~isempty(lik.p.nu)
        % dfdfdnu
        i1 = i1+1;
        llg3(:,i1) = (r.^2-2.*v.*sigma2-sigma2)./(v.*sigma2 + r.^2).^2 - 2.*sigma2.*(r.^2-v.*sigma2).*(v+1)./(v.*sigma2 + r.^2).^3;
        llg3(:,i1) = llg3(:,i1).*v.*log(v);
      end
  end
end
