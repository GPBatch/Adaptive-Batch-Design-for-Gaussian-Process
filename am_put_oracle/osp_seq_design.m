...
%       (FB/MLB/RB/ABSUR) to estimate the stoppint criteria
    ...
    if ( strcmp(design, 'lhs') )
       ...
       % initial conditions for all the forward paths: replicated design with km.batch
        ...
        while (add_more_sites)
            ...
            % design batch
            [add_grid, r_cur, gamma, emp_loss(i,k-model.init_size)] = seq_design(fits(i), all_X(1:k-1,1:d), all_X(1:k-1,d+1), all_X(1:k-1,d+2), all_X(1:k-1,end), model, lhs_rect, i, gamma);
